...
/*
解法：统计最长的子等差数列，长度减2，然后使用公式num*（num+1）/2；加和所有的子数组即可。不过这里dp数组是没什么用的，在if语句里，直接num++就行了……
...
/*
解法二：解法一想麻烦了，假如遇到了新的A[i]，并且与之前的组成了等差数列，那么只需要加上A[i]参与的数列数即可，很显然就是【子数列长度-2】;或者把dp[i]理解为以A[i]结尾的等差列个数。
执行用时：0 ms, 在所有 Java 提交中击败了100.00% 的用户
