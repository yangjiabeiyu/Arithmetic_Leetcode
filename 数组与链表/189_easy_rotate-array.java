...
/*
解法三：数组从元素(pos,ele)开始，找应该属于自己的座位pos+k；pos+k处的元素离开去找属于自己的座位，不断循环；如果形成一个环状，即走到了ele的位置，则开始下一个元素的换座。
由于最多换位n次，所以可设置计数来终止数组遍历。
执行用时：0 ms, 在所有 Java 提交中击败了100.00% 的用户
内存消耗：39.3 MB, 在所有 Java 提交中击败了67.40% 的用户
*/
class Solution {
    public void rotate(int[] nums, int k) {
        if(nums.length <= 1)
